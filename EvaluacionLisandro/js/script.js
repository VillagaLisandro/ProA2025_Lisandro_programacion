...
let highScore = 0;  // guarda puntuacion mas alta

//carga sus img
dinoImg.src = "img/dino.png";
cactusImg.src = "img/cactus.png";

// boton de reinicio manual
const manualRestartBtn = document.createElement("button");
manualRestartBtn.id = "manualRestartBtn";
manualRestartBtn.textContent = "Reiniciar Manual";
document.body.appendChild(manualRestartBtn);

// evento para el reinicio manual
manualRestartBtn.addEventListener("click", function() {
    gameOver = false;
    score = 0;
    gameSpeed = 5;
    dino.y = canvas.height - dino.height;
...
update = function() {
    ...
    originalUpdate.apply(this, arguments);  // Ejecutar lógica original
...
draw = function() {
    ...
    // Sobrescribe color si el dino está saltando
    if (dino.isJumping) {
        ctx.clearRect(dino.x, dino.y, dino.width, dino.height);
        ctx.fillStyle = "orange";  // Color cuando está en el aire
        ctx.fillRect(dino.x, dino.y, dino.width, dino.height);
    }
};
// Obtiene el elemento de audio desde el HTML
let jumpSound = document.getElementById("jumpAudio");
